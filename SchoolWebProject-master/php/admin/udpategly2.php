...
if($username=="" || $password==""){
	...
	$sql2=mysqli_query($con,"select *  from school.user where `username`='".$username."'");//条件是接收用户名称来查库的用户
	$row2=mysqli_fetch_row($sql2);
	$sql=mysqli_query($con,"select *  from school.user where `id`='".$id."'");//条件是id来查库的用户
	if(mysqli_num_rows($sql)){//判断用户名冲突
		$row=mysqli_fetch_row($sql);
		if($row[1]==$username){//判断是传过来的用户名和库查出来的是否一致，一致就可以直接改其他信息
			mysqli_query($con,"UPDATE  `school`.`user` SET  `password` =  '".$password."',`adminlevel` =  '".$level."' WHERE  `user`.`username`='".$username."'");//执行mysqli语句，更新管理员用户
		...
		if(isset($_SESSION['id'])){
			$id=$_SESSION["id"];
			mysqli_query($con,"UPDATE  `school`.`user` SET  `username` =  '".$username."',`password` =  '".$password."',`adminlevel` =  '".$level."' WHERE  `user`.`id` =".$id."");
		}	
