...
if($title=="" || $autor=="" || $content==""){//大判断是否为空
	...
	/*连接数据库上传数据*/
	$sql=mysqli_query($con,"insert into `school`.`news`(`classname`,`title`,`autor`,`content`,`thumbnail`,`updatetime`)values('".$classname."','".$title."','".$autor."','".$content."','".$lujing."','".$updatetime."')");
	if($sql){
