...
if($username=="" || $password==""){
	...
	$sql=mysqli_query($con,"select *  from school.user where username='".$username."'");//查询库的用户
	if(mysqli_num_rows($sql)){
		echo '<script>alert("已有该用户名称，请重新创建新的用户名称");history.back();</script>';	//提示已存在
		mysqli_close($con);//关库
	}else{
		mysqli_query($con,"INSERT INTO  `school`.`user` (`username` ,`password` ,`adminlevel`)VALUES ('".$username."',  '".$password."',  '".$level."')");//添加管理员。执行mysqli语句
