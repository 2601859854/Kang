...
if($password=="" || $xmm==""){
	...
	$sql=mysqli_query($con,"select *  from school.user where username='".$username."' and password='".$password."'");//查询库的用户使用
	if(mysqli_num_rows($sql)){	
		$sql2=mysqli_query($con,"UPDATE  `school`.`user` SET  `password` =  '".$xmm."' WHERE  `user`.`username` ='".$username."'");
		if($sql2){
