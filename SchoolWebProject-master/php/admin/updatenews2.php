...
if($title=="" || $autor=="" || $content==""){
	...
	$sql=mysqli_query($con,"select *  from school.news where `id`='".$id."'");//条件是id来查库的新闻
	if(mysqli_num_rows($sql)){//判断新闻标题冲突
		$row=mysqli_fetch_row($sql);
		if($row[2]==$title){//判断是传过来的新闻标题和库查出来的是否一致，一致就可以直接改其他信息
			mysqli_query($con,"UPDATE  `school`.`news` SET  `classname` =  '".$classname."',`autor` =  '".$autor."',`content`='".$content."',`updatetime`='".$updatetime."' WHERE  `news`.`title`='".$title."'");//执行mysqli语句，修改新闻
		...
		if(isset($_SESSION['id'])){
			$id=$_SESSION["id"];
			mysqli_query($con,"UPDATE  `school`.`news` SET  `classname` =  '".$classname."',`autor` =  '".$autor."',`content` =  '".$content."',`title`='".$title."',`updatetime`='".$updatetime."' WHERE  `news`.`id` =".$id."");
		}	
